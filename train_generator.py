...
def get_opt():
    ...
    # Cuda availability - Fixed to be a proper flag
...
def train(opt, train_loader, test_loader, test_vis_loader, board, tocg, generator, discriminator, model, device):
    """
        Train Generator
    """

    # Model - Move to dynamic device
    if not opt.GT:
        tocg.to(device)
        tocg.eval()
    ...
    if opt.fp16:
        ...
        # Respect cuda flag for GANLoss tensor type
        criterionGAN = GANLoss('hinge', tensor=torch.cuda.FloatTensor if device.type == 'cuda' else torch.FloatTensor)
        
    criterionFeat = nn.L1Loss()
    criterionVGG = VGGLoss(opt).to(device) # Move VGG to device

    # optimizer
    optimizer_gen = torch.optim.Adam(generator.parameters(), lr=opt.G_lr, betas=(0.0, 0.9))
    ...
    gauss = tgm.image.GaussianBlur((15, 15), (3, 3)).to(device) # Move gauss to device

    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        iter_start_time = time.time()
        inputs = train_loader.next_batch()

        # input - Move to dynamic device
        agnostic = inputs['agnostic'].to(device)
        parse_GT = inputs['parse'].to(device)
        pose = inputs['densepose'].to(device)
        parse_cloth = inputs['parse_cloth'].to(device)
        parse_agnostic = inputs['parse_agnostic'].to(device)
        pcm = inputs['pcm'].to(device)
        ...
        with torch.no_grad():
            if not opt.GT:
                ...
                # warped cloth mask one hot - Move to dynamic device
                warped_cm_onehot = torch.tensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(np.float32), device=device)
            ...
            # Create parse maps on dynamic device
        ...
        if not opt.no_ganFeat_loss:
            num_D = len(pred_fake)
            # Use dynamic device for GAN_Feat_loss
            GAN_Feat_loss = torch.zeros(len(opt.gpu_ids) if len(opt.gpu_ids) > 0 else 1, device=device)
        ...
        if (step + 1) % opt.tensorboard_count == 0:
            ...
            c_paired = inputs['cloth']['unpaired'].to(device)
            im = inputs['image'].to(device)
            ...
            with torch.no_grad():
                ...
                parse = torch.zeros(fake_parse.size(0), 7, opt.fine_height, opt.fine_width, device=device)
                for i in range(len(labels)):
                    for label_id in labels[i][1]:
                        parse[:, i] += old_parse[:, label_id]
                parse = parse.detach()
...
def main():
    ...
    if not opt.GT:
        ...
        if opt.tocg_checkpoint:
            load_checkpoint(tocg, opt.tocg_checkpoint, opt) # Pass opt to load_checkpoint
    ...
    # Move model to device and respect opt.cuda
    model = models.PerceptualLoss(model='net-lin',net='alex',use_gpu=opt.cuda)
    ...
    # Pass device to train function
    train(opt, train_loader, test_loader, test_vis_loader, board, tocg, generator, discriminator, model, device)
